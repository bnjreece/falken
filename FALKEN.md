...
- **Keys** (primary = Opt-letter, fallback = C-a+Shift): `Opt-s` switcher (inside: `^F` cycle filter all/active/dormant · `f:…` custom · `^W` wrap-toggle · `^X` kill) · `Opt-e` cycle the active filter inline · `Opt-n` next needs · `Opt-m` cycle done · `Opt-w` wrap⇄unwrap toggle · `Opt-i` help · `C-a W`/`C-a U` force wrap/unwrap.
